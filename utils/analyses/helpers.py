...
def aggregate_dimensions(concept_embedding: np.ndarray, idx_triplet: np.ndarray) -> np.ndarray:
...
def get_max_dims(concept_embedding: np.ndarray, triplets: np.ndarray) -> np.ndarray:
    ...
    def get_max_dim(triplet: np.ndarray) -> np.ndarray:
...
def get_topk_dims(concept_embedding: np.ndarray, triplets: np.ndarray, k: int = 2) -> np.ndarray:
    """Get top-k most important dimension for the most similar object pair in a triplet."""
    aggregate = partial(aggregate_dimensions, concept_embedding)

    def get_topks(k: int, triplet: np.ndarray) -> np.ndarray:
...
def get_failures(triplets: np.ndarray, model_choices: np.ndarray, target: int = 2) -> np.ndarray:
...
def get_family_name(model_name: str) -> str:
    ...
    for children in all_children:
        if getattr(families, children):
            ...
            if family_name not in ["CNN", "SSL"]:
                return family_name

    # Fallback logic for known model types
    if "resnet" in model_name.lower():
        return "CNN"
    if "clip" in model_name.lower() or "openclip" in model_name.lower():
        return "CLIP"
    if "vit" in model_name.lower():
        return "ViT"
    if "moco" in model_name.lower():
        return "SSL"
    if "barlow" in model_name.lower() or "simclr" in model_name.lower():
        return "SSL"
